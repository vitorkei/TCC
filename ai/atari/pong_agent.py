...
units            = 256
batch_size       = 32
conv_params      = [[32, 64, 64], # conv filters
                    [8, 4, 3],    # kernel size
                    [4, 2, 1]]    # stride
...
def preprocess(frame):
    gray = rgb2gray(frame)
    cropped_frame = gray[34:-16, 0:] # 161x160 pixels
...
def translate_action(action):
  if np.array_equal(action, np.array([1, 0])): # UP
    return 2
  elif np.array_equal(action, np.array([0, 1])): # DOWN
    return 3
