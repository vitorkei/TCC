...
if __name__ == "__main__":
  ...
  # Instancia das classes DQNetwork e Memory
  ...
  for i in range(pretrain_length):
    ...
    # o jogo acabou ou não, após tomar um ação aleatória    choice = random.randint(0, len(possible_actions)-1)
    action = possible_actions[choice]
        ...
        while step < max_steps:
          ...
          for i in range(len(batch)):
            ...
            if terminal:
              ...
              target = reward_mb[i] + gamma * np.max(Qs_next_state[i])
              targe_Qs_batch.append(target)
