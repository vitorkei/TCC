...
\footnotesize Traduções livres feitas pelo autor
...
Aprendizado de máquina, ou \textit{machine learning} (ML) como é mais conhecido, é um campo de ciência da computação e um dos ramos de inteligência artificial que estuda a capacidade dos computadores de aprender a realizar uma tarefa a partir de dados, sem ser explicitamente programado para executá-la. Após ter a tarefa que deve ser cumprida pelo computador definida, o programa recebe uma grande quantidade de dados, tenta reconhecer um padrão nessa entrada e, por fim, constrói um modelo para realizar predições.
...
Aprendizado supervisionado consiste em dar uma tarefa, uma entrada e uma saída esperada para o computador. A entrada e a saída são grandes volumes de dados e respectivas categorias, rótulos. Em outras palavras, a máquina tem um "professor" que diz se a resposta dada está certa ou errada. Um exemplo simples que ajuda a ilustrar como aprendizado supervisionado funciona é fazer um computador dizer se uma imagem é de um cachorro ou não. Após mostrar diversas imagens de cachorro, dizendo quais são cachorros e quais não são, o computador deve conseguir ver uma imagem e dizer se é um ou não. 

Esta técnica cai em dois tipos de problemas: classificação e regressão.
Em problemas de regressão, é esperado que os dados de entrada sigam uma função $g$, e o algoritmo deve encontrar uma função $h$ que se aproxime o melhor possível de $g$. Um exemplo deste tipo de problema é fornecer dados sobre uma residência, como a metragem, e dizer seu valor. Com entrada suficiente, o programa deve conseguir dizer o valor de uma casa ou apartamento apenas pelas medidas.
Em problemas de classificação, deseja-se que o computador classifique corretamente uma entrada, dentre uma série de categorias pré-determinadas. Um exemplo simples deste problema é o fornecido anteriormente, de classificar uma imagem se é de cachorro ou não.
Um problema pode ser tanto de classificação quanto de regressão a depender de como for montado. Ao invés de o programa dizer o preço do imóvel, ele poderia dizer se o preço é superior ou inferior a um certo valor.

Redes neurais (\textit{neural networks}), Máquinas de Vetor de Suporte (\textit{Support Vector Machine}, SVM) e Classificadores \textit{Naive Bayes} são alguns dos algoritmos de aprendizado supervisionado mais comuns. O foco será em redes neurais.
...
A entrada é o que a IA recebe inicialmente e precisa processar; as camadas ocultas (\textit{hidden layers}) são o processamento; e a saída é a decisão tomada pela IA. No caso este trabalho, a entrada é o \textit{frame} atual do jogo e a saída é a ação tomada (mover-se, atirar e etc).
A saída, assim como a entrada e as camadas ocultas, é composta por neurônios, mas que representam a probabilidade de cada decisão ser tomada. Para a IA, existem 8 neurônios na saída, um para cada ação (ainda que não seja diretamente relacionada ao jogo, como \textit{RESET} e \textit{null}) e a respectiva probabilidade de ela ser tomada dado o \textit{frame} de entrada.
...
Cada neurônio das camadas ocultas representa uma característica (\textit{feature}) desejada pelo desenvolvedor da IA. Se essa característica estiver presente na entrada, então esse neurônio será \textbf{ativado}. A ativação de um ou mais neurônios pode levar a ativação de outros neurônios na próxima camada, que é um comportamento inspirado na forma como neurônios do cérebro enviam sinais de um para o outro. No caso das redes neurais, um neurônio é ativado quando a soma dos números de entrada passam de um certo valor. Porém, nem todos os valores de entrada devem ser igualmente importantes, então cada um desses números recebe um peso que determina sua importância para a ativação da característica.

Em forma matemática, seja $n$ o número de neurônios na camada anterior, $w_{i}, i = 1, ..., n$ os pesos das saídas de cada neurônio da camada anterior, e $a_{i}, i = 1, ..., n$ o valor de saída de cada neurônio da camada anterior e, por consequência, cada valor de entrada do neurônio atual, e $b$ o valor que a soma deve ultrapassar para que o neurônio seja ativado.
