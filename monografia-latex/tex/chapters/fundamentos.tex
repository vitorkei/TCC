...
Para domínios mais simples, como Jogo da velha, é possível determinar qual a ação com maior recompensa esperada para cada estado - ou seja, a ação com maior probabilidade de vencer o jogo. Conforme o domínio se torna mais complexo, fazer esse mapeamento se torna inviável por conta da quantidade de dados que precisam ser armazenado, como é o caso do jogo \textit{Asteroids}. Além disso, não é raro haver casos em que não é possível dizer se uma ação tomada deve ter recompensa positiva ou negativa. Nesses casos, é mais viável criar e treinar um agente que aprenda a se comportar no ambiente em que está inserido do que informar se cada uma de suas ações em cada um dos estados possíveis é boa ou ruim.

Para tomar as decisões com melhor resultado, o agente precisa de NÚMERO informações
