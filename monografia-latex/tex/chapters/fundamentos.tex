...
%    \thead[l]{\textit{O empolgante novo esforço de fazer} \\ \textit{computadores pensarem, serem máquinas} \\ \textit{com mentes, no sentido completo e literal} \\ (Haugeland, 1986) \\ \\ \textit{[A automação de] atividades} \\ \texit{que são associadas ao pensamento} \\ \textit{humano, como resolução de} \\ \textit{de problemas, tomada de decisão,} \\ \textit{aprendizado...} \\ (Hellman, 1978)} & \thead[l]{\textit{O estudo das faculdades mentais através} \\ \textit{de modelos computacionais} \\ (Charniak \& McDermott, 1985) \\ \\ \textit{O estudo das computações que tornam possível} \\ \textit{a percepção, razão, e ação} \\ (Winston, 1992)} \\ \hline
%    \textbf{Agindo como um huamno} & \textbf{Agindo racionalmente} \\ \hline
%    \thead[l]{\textit{A arte de criar máquinas que realizam} \\ \textit{funções que requerem inteligência quando feitas} \\ \textit{por pessoas} \\ (Kurzweil, 1990)} & bla \\ \hline
%  \end{tabular}
...
\begin{tabular}{| p{7cm} | p{7cm} |}
  ...
  \textbf{Agindo como um humano} & \textbf{Agindo radionalmente} \\ \hline
