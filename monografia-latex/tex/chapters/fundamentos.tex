...
Neurônios são objetos que armazenam um número, em geral entre 0 e 1 inclusos, cujo significado varia de acordo com o trabalho em questão. No caso deste, os neurônios da primeira camada armazenam o número que representa cada pixel da imagem, após serem convertidos para uma escala de cinza e em um número entre 0 e 1. Os quadros (\textit{frames}) de entrada também são redimensionados antes de serem analisados e processados pela IA.
...
A entrada é o que a IA recebe inicialmente e precisa processar, as camadas ocultas (\textit{hidden layers}) bem como as transições para elas e entre elas são o processamento, e a saída é a decisão tomada pela IA. No caso este trabalho, a entrada é o \textit{frame} atual do jogo e a saída é a ação tomada (mover-se, atirar e etc).
