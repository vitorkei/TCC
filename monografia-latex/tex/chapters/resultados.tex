...
Os gráficos não se mostraram informativos, então alguns resultados serão apresentados por uma tabela.
A comparação foi feita apenas entre agente aleatório e agente treinado por \textit{deep Q-learning}.
...
Nos experimentos, foram coletadas a pontuação final média de uma pessoa experiente após uma hora de jogo, de um agente jogando aleatóriamente, e do agente jogando segundo o modelo que montou no treinamento, todos contra a inteligência artificial padrão do \textit{Pong}.
...
\begin{figure}[h!]
  ...
  \end{minipage}
  \caption{A imagem da esquerda mostra a pontuação ao longo do aprendizado e a da direita de um agente aleatório.}
  \label{fig:asteroids_score}
\end{figure}

Percebe-se que o agente não conseguiu aprender ou sequer mostrar indícios de melhora mesmo com um tempo de aprendizado (em \textit{frames}) próximo do \textit{Pong}.
...
A pontuação de jogadores experientes não foi considerada por estar muito acima, passando com facilidade dos 30000 pontos.
